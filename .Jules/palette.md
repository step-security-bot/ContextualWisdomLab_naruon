...
## 2026-06-26 - Add title tooltips to disabled buttons
**Learning:** Buttons that are disabled for specific reasons (e.g. read-only calendar, missing draft, missing input) can be confusing for users if the reason is not obvious. Adding a `title` tooltip to explain *why* the button is disabled is a small but valuable UX enhancement.
**Action:** When creating or updating a button that can be disabled due to specific conditions, add a `title` attribute providing a clear explanation of why it is disabled (e.g., `title={!draft ? '작성된 초안이 없습니다' : undefined}`).
