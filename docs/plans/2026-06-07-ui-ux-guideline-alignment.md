...
- `TasksLayout` now derives ticket board/list/detail rows from `/api/tasks`, routes creation to `/mail`, routes delegated follow-up to `/search`, performs PATCH status updates, renders overdue reply follow-up and self-sent knowledge materialization as signed API actions, and strips HTML-like title markup from visible and accessible labels.
